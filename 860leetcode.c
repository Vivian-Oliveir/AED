...
int main(){
    int *bills, i = 0, j, op;
    bills = (int *)malloc(i * sizeof(int));

    do {
        op = 0;
        j = i + 1;
        bills = realloc(bills, j * sizeof(int));
        printf("Digite a %d nota: ", i + 1);
        scanf("%d", (bills+i));
        i++;
        printf("Deseja adicionar outra nota? (1. Sim 2. Não)");
        scanf("%d", &op);
    } while( op == 1 );

    if( lemonadeChange(bills, j) ) {
        printf("True.");
    } else {
        printf("False.");
    }
